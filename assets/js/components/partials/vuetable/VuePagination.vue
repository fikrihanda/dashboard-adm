      ...
      <template v-else>
        <template v-for="n in windowSize">
          <li :class="['page-item', isCurrentPage(windowStart+n-1) ? 'active' : '']">
            <span v-if="isCurrentPage(windowStart+n-1)" class="page-link" v-html="n"></span>
            <button v-else type="button" class="page-link" v-html="n" @click="loadPage(windowStart+n-1)"></button>
          </li>
        </template>
      </template>
